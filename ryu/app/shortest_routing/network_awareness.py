...
class NetworkAwareness(app_manager.RyuApp):
	...
	def __init__(self, *args, **kwargs):
		...
		self.discover_thread = hub.spawn(self._discover)  #开启一个进程，用以不停的获取网络信息

	def _discover(self):   #SDN控制器和网络链接后SDN控制器不可能立刻获得网络信息，需要等待一段时间初始化
		...
		while True:
			...
			if i == 2:   # Reload topology every 20 seconds.
				...
				self.signal = 1
				self.get_topology(None)  #获得网络相关信息
	...
	def _packet_in_handler(self, ev):  #network_awareness同样有paket_in函数，很正常，因为SDN控制器要获得网络信息，一定需要来自网络的数据
									#这些数据同样可以触发packet_in，然后就可以获得诸如两个交换机是否所连接关系等信息了
		"""
	...
	def get_topology(self, ev):  #获取网络相关信息，以及需要的一些数据结构
		...
		switch_list = get_switch(self.topology_api_app, None) #由ryu的topo获取api获得底层网络的交换机列表
		self.create_port_map(switch_list)
		self.switches = [sw.dp.id for sw in switch_list]
		links = get_link(self.topology_api_app, None) #由ryu的topo获取api获得底层网络的链路信息
		self.create_interior_links(links)
		self.create_access_ports()
		self.graph = self.get_graph(self.link_to_port.keys()) #由networkX根据交换机和链路信息获得网络的图结构
	...
	def get_host_location(self, host_ip): #获得主机直连的交换机
		"""
	...
	def k_shortest_paths(self, graph, src, dst, weight='weight', k=5):#获得src和dst之间的k条最短路径
		"""
	...
	def all_k_shortest_paths(self, graph, weight='weight', k=5):#获取每对节点之间所有的k条最短路径
		"""
	...
	def show_topology(self): #展示网络信息，默认不展示
