...
* fattree is the topology modules;
...
And then, go into the top directory of Ryu, and run the application. You are suggested to add arguments when starting Ryu. An example is shown below:
    ...
    $ cd ryu
    $ ryu-manager --observe-links 当前文件夹下/PureSDN2.py ofctl_rest.py rest_topology.py --k_paths=4 --weight=hop --fanout=4
...
you can user ping or inperf to produce flow:
for example:

    mininet> h001 ping h005
    mininet> iperf h001 h004

or you can use xterm to start hosts:
   
    mininet> xterm h001 h005
