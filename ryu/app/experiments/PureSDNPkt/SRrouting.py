...
class ShortestForwarding(app_manager.RyuApp):
    ...
    def add_flow(self, dp, priority, match, actions, idle_timeout=0, hard_timeout=0):
        ...
        mod = parser.OFPFlowMod(datapath=dp, priority=priority,flags=ofproto.OFPFF_SEND_FLOW_REM,
    ...
    def flow_removed_handler(self, ev):
        ...
        deleteInfo = {ip_src, ip_dst, protocol, l4port1, l4port2}
    ...
    def arp_forwarding(self, msg, src_ip, dst_ip):
        ...
        if result:
            ...
            # Flood is not good.
            self.flood(msg)
    def Segment_forwarding(self,flow_info,last_dp,bucket,inport,msg,bool=False):

        datapath=bucket[0]
        segmentStack=bucket[1]
        ofproto=datapath.ofproto
        parser = datapath.ofproto_parser
        last_output=segmentStack[-1]
        ...
        first_outport=segmentStack.pop()
        actions.append(parser.OFPActionOutput(first_outport,0))
        inst = [parser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS, actions)]
        if len(flow_info)==7: #tcp packet
            match = parser.OFPMatch(
                eth_type=flow_info[0], ipv4_src=flow_info[1], ipv4_dst=flow_info[2],in_port=flow_info[3],
                ip_proto=flow_info[4], tcp_src=flow_info[-1][0], tcp_dst=flow_info[-1][1]
            )
        elif len(flow_info)==5: #icmp packet
            match = parser.OFPMatch(
                eth_type=flow_info[0], ipv4_src=flow_info[2], ipv4_dst=flow_info[3], in_port=flow_info[-1],
                ip_proto=flow_info[1]
            )
        ...
        # print str(datapath, flow_info, first_data_Stack, inport, msg)
        if bool:
            # print str(datapath, flow_info,first_data_Stack,inport,msg)
            print 'send packet out'
            print 'last dp is %d, inport is %s, lastoutput is %s',last_dp.id,inport,last_output
            self.send_packet_out(last_dp, msg.buffer_id, inport, last_output, msg.data)
    ...
    def icmp_forwarding(self,msg,ip_protocol,eth_type,ip_src,ip_dst):
        ...
        flow_info=(eth_type,ip_protocol,ip_src,ip_dst,in_port)
        flow_info_reverse=(eth_type,ip_protocol,ip_dst,ip_src,dst_port)
        ...
        if src_sw==dst_sw:
            ...
            if path == None:
                return
            last_dp = self.datapaths[path[len(path)-2]]
            path.reverse()
            try:
                bucket=self.swToSegments(path)
                self.Segment_forwarding(flow_info_reverse,last_dp,bucket,in_port,msg,False)
                path.reverse()
                bucket=self.swToSegments(path)
                self.Segment_forwarding(flow_info,last_dp,bucket,in_port,msg,True)
                # print (first_dp, msg.buffer_id, in_port, bucket[0], msg.data)
            except:
                self.flood(msg)
    ...
    def shortest_forwarding(self, msg, eth_type, ip_src, ip_dst):
        ...
        if src_sw == dst_sw:
            ...
            try:
                ...
                self.flood(msg)
    def compute_runing_time(self):
        self.stepTimeStamp = time.time()
        period=0
        alltime=0
        if self.firstTimeStamp != 0 and self.stepTimeStamp != 0 and self.stepTimeStamp > self.firstTimeStamp:
            period = self.stepTimeStamp - self.firstTimeStamp
        if self.timeStamp != 0 and self.stepTimeStamp != 0 and self.stepTimeStamp > self.timeStamp:
            alltime = self.stepTimeStamp - self.timeStamp
        self.timeStampList.append(period)
        self.allTimeList.append(alltime)
        print '#######################'
        print self.allTimeList

    def _bandwidth_demand(self,ip_src,ip_dst):
        src_count = self.newComingFlows['src'][ip_src]
        dst_count = self.newComingFlows['dst'][ip_dst]
        try:
            demand1=setting.host_Capacity/src_count
            demand2=setting.host_Capacity/dst_count
        except:
            demand1=setting.host_Capacity
            demand2=setting.host_Capacity
        if demand1>demand2:
            return demand2
        else:
            return demand1

    def _select_paths1(self, flowDemand, paths):
        pathDict = {}
        temp_paths = []
        sumBwofPaths = 0.0
        maxLossofPaths = -1
        ...
        for path in paths:
            ...
            free = CONF.miceIndex * temp[0] - flowDemand
            if free >= 0:
                temp_paths.append(p)
                pathDict.setdefault(p, [])
                pathDict[p].append(free)  # {path:[bw,d,loss,errors]}
                pathDict[p].append(temp[1])
                pathDict[p].append(temp[2])
                if maxLossofPaths < temp[1]:
                    maxLossofPaths = temp[1]
                if minLossofPaths > temp[1]:
                    minLossofPaths = temp[1]
                if pathMaxBwWave < temp[2]:
                    pathMaxBwWave = temp[2]
                if pathMinBwWave > temp[2]:
                    pathMinBwWave = temp[2]
        ...
        try:
            for p in temp_paths:
                a = setting.a
                c1 = (maxLossofPaths - pathDict[p][1]) / (maxLossofPaths - minLossofPaths + 0.0001)
                c2 = (pathMaxBwWave - pathDict[p][2]) / (pathMaxBwWave - pathMinBwWave)
                c = a * c1 + (1 - a) * c2
                if maxWeight < c:
                    maxWeight = c
                    bestPath = p
            ...
            return randomPath
    def _select_paths(self,src_sw,dst_sw,flowDemand):
        paths = self.awareness.shortest_paths.get(src_sw).get(dst_sw)
        pathDict = {}
        sumBwofPaths = 0.0
        sumLossofPaths = 0.001
        sumErrorofPaths = 0.001
        pathBwWave = 0.0
        self.graph = self.monitor.graph
        maxbwOfpath = 0
        maxbwPath = []
        ...
        for path in paths:
            ...
            if free >= 0:
                ...
                pathDict[p].append(temp[1])
                pathDict[p].append(temp[2])
                pathDict[p].append(temp[3])
                sumBwofPaths += setting.host_Capacity / free
                sumLossofPaths += temp[1]
                sumErrorofPaths += temp[2]
                pathBwWave += temp[3]
        if pathDict == {}:
            return maxbwPath
        bottom = 0
        try:
            for p in pathDict:
                ...
                a=setting.a
                c = setting.c
                c1 = pathDict[p][0] / sumBwofPaths
                c2 = pathDict[p][1] / pathBwWave
                c3 = pathDict[p][2] / sumLossofPaths
                c4 = pathDict[p][3] / sumErrorofPaths
                c = 1 / ((a * c2 + (1 - a) * (c * c3 + (1 - c) * c4)))
                pathDict[p].append(c)
                bottom += c
            r = random.random()
            cp = 0.0
    ...
    def get_minBand_ofPath(self, path):
        ...
        if _len > 1:
            ...
            for i in xrange(_len - 1):
                ...
                allBw += bw
                loss = self.graph[pre][curr]['loss']
                accuLoss = accuLoss * (1 - loss)
                if minBw > bw:
                    minBw = bw
                waveBw.append(bw)
            ...
            for b in waveBw:
                sd = sd + (b - averageBw) ** 2
        return minBw, 1 - accuLoss, pow(sd, 0.5)
