...
parser.add_argument('--fsize', dest='fsize', default='1G', help="Number of iperf flows per host")
...
def create_peers():
	...
	if args.traffic.startswith('stag'):
		...
		flows_peers = create_random_peers(HostList, int(args.flows_num_per_host))
	else:
		pass
	...
	flow_peers=[]
	for peer in flows_peers:
		client=peer[0]
		server=peer[1]
		temp_peer='%s-%s-%s'%(str(client),str(server),str(args.fsize))
		flow_peers.append(temp_peer)
	print flow_peers
	file_save = open('peers/iperf_peers_fsize1.txt', 'w')
	file_save.write(str(flow_peers)[1:-1])
	file_save.close()
