...
traffics="random"
round="3 4 5 6 7 8"
mice="0.6 0.7 0.8 0.9 1.0"
out_dir=./miceIndex_Result_random
mkdir -p $out_dir
# Run experiments.
sudo mn -c
for r in $round
  ...
  mkdir -p $dir1
  sleep 1
  sudo python ./PureSDNPkt/SegmentFattree.py --k $k --dir $dir1 --fnum $flows_num_per_host --duration $duration --miceIndex $m
  done
done
