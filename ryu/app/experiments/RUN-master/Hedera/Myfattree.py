...
def install_proactive(net, topo):
    ...
    for sw in topo.EdgeSwitchList:
        ...
        if topo.pod == 4:
			...
			'group_id=1,type=select,bucket=actions=output:1,bucket=actions=output:2'" % sw  # origin is 1 and 2,i think wrong
        elif topo.pod == 8:
            cmd = "ovs-ofctl add-group %s -O OpenFlow13 \
        ...
        for i in Edge_List:
            if i != num:
                ...
                cmd = "ovs-ofctl add-flow %s -O OpenFlow13 \
				'table=0,idle_timeout=0,hard_timeout=0,priority=10,arp,\
				nw_src=10.%d.0.0/16,nw_dst=10.%d.0.0/16,actions=group:1'" % (sw, num,  i)
                os.system(cmd)
                cmd = "ovs-ofctl add-flow %s -O OpenFlow13 \
				'table=0,idle_timeout=0,hard_timeout=0,priority=10,ip,\
				nw_src=10.%d.0.0/16,nw_dst=10.%d.0.0/16,actions=group:1'" % (sw, num,  i)
    ...
    for sw in topo.AggSwitchList:
        ...
        if topo.pod == 4:
            cmd = "ovs-ofctl add-group %s -O OpenFlow13 \
			'group_id=1,type=select,bucket=actions=output:1,bucket=actions=output:2'" % sw
        elif topo.pod == 8:
            cmd = "ovs-ofctl add-group %s -O OpenFlow13 \
...
def run_experiment(pod, density, ip="192.168.229.128", port=6653, bw_c2a=10, bw_a2e=10, bw_e2h=10):
    ...
    fanout = args.k
    #Controller_Ryu = Popen("ryu-manager --observe-links ./NECMP_demo1.py --k_paths=%d --weight=hop --fanout=%d" % (k_paths, fanout), shell=True, preexec_fn=os.setsid)
