...
class ShortestForwarding(app_manager.RyuApp):
	...
	def _packet_in_handler(self, ev):
		...
		if isinstance(ip_pkt, ipv4.ipv4):
			...
			if len(pkt.get_protocols(ethernet.ethernet)):
				...
				#self.SegmentRouting(msg,eth_type,ip_pkt.src,ip_pkt.dst)


	def Segment_forwarding(self,datapath,flow_info,portList):
		...
		while len(portList)>1:
			...
			# actions.append(parser.OFPActionSetField(f_tc))
			# actions.append(parser.OFPActionSetMplsTtl(mpls_ttl))

		actions.append(parser.OFPActionOutput(portList.pop(),0))
	...
	def send_flow_mod(self, datapath, flow_info, src_port, dst_port):
		...
		parser = datapath.ofproto_parser
		actions = []
		# actions.append(parser.OFPActionPushMpls(123))
	...
	def install_flow(self, datapaths , dst_sw,link_to_port, path, flow_info, buffer_id, data=None):
		'''
		...
		self.Segment_forwarding(first_dp,flow_info,portList)
		#  Install flow entry for the last datapath.

		#  Install flow entry for the first datapath.


	def get_L4_info2(self, tcp_pkt, udp_pkt):
		"""
		...
		if result:
			...
			if dst_sw:
				...
				if setting.enable_Flow_Entry_L4Port:
					...
					# Get ip_proto and L4 port number.
					ip_proto, L4_port, Flag = self.get_L4_info2(tcp_pkt, udp_pkt)
				...
				self.install_flow(self.datapaths , dst_sw,
