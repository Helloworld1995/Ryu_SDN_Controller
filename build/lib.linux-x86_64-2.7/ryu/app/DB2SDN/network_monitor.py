...
class NetworkMonitor(app_manager.RyuApp):
	...
	def  get_min_bw_of_links(self, graph, path, min_bw):
		...
		if _len > 1:
			...
			return min_bw
	def get_best_bw_path(self,graph,paths):
		max = 0
		for path in paths:
			temp=self.get_minBand_ofPath(graph,path)
			if(temp>max):
				max=temp
				bestPath=path

		return bestPath
	def  get_minBand_ofPath(self, graph, path):
		"""
			Getting bandwidth of path. Actually, the mininum bandwidth
			of links is the path's bandwith, because it is the bottleneck of path.
		"""
			...
			for link in link_to_port:
				...
				if src_dpid in bw_dict and dst_dpid in bw_dict:
					...
					if graph.has_edge(src_dpid, dst_dpid):
						...
						graph[src_dpid][dst_dpid]['BandWeight']=math.floor(setting.MAX_CAPACITY/bandwidth)
				# 	else:
				...
				# 		graph[src_dpid][dst_dpid]['BandWeight'] = setting.MAX_CAPACITY
				# 	else:
				# 		graph.add_edge(src_dpid, dst_dpid)
