...
class ShortestForwarding(app_manager.RyuApp):
	...
	def _packet_in_handler(self, ev):
		...
		if isinstance(ip_pkt, ipv4.ipv4):
			...
			print("ip_adress is %s---->"%ip_pkt.src)
			result1 = userDao.Dbutil().select(str(ip_pkt.src))
			if result1 == None:
				return
			id = result1[0]
			print(id)
			if len(pkt.get_protocols(ethernet.ethernet)):
				...
				self.shortest_forwarding(id,msg, eth_type, ip_pkt.src, ip_pkt.dst)
	...
	def shortest_forwarding(self, id,msg, eth_type, ip_src, ip_dst):
		"""
		...
		if result:
			...
			if dst_sw:
				...
				userDao.Dbutil().insert(id,ip_src,ip_dst,str(path)) #insert path message into database
