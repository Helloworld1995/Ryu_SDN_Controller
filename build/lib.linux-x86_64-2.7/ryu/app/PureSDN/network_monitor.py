...
class NetworkMonitor(app_manager.RyuApp):
	...
	def _flow_stats_reply_handler(self, ev):
		...
		for stat in sorted([flow for flow in body if (
				(flow.priority not in [0, 20,65535]) and (flow.match.get('ipv4_src')) and (flow.match.get('ipv4_dst')))],
			...
			if flowDemand >0.1:
				...
				self.flows[key]={'src': src, 'dst': dst, 'demand': flowDemand,'speed':speed,'converged': False,'receiver_limited': False,'match':stat.match,'priority': stat.priority}
	...
	def _ip2sw(self,src,dst):
		src_dp = self.awareness.get_host_location(src)[0]
		dst_dp = self.awareness.get_host_location(dst)[0]
	...
	def _port_stats_reply_handler(self, ev):
		...
		if len(self.current_dectect_dp) == 1.25 * (CONF.fanout ** 2) and self.current_free_bandwidth!=None:
			self.free_bandwidth=self.current_free_bandwidth.copy()
	...
	def create_static_bw_graph(self):
		...
		try:
			...
			for link in graph.edges():
				node1=link[0]
				node2=link[1]
				graph[node1][node2]['bandwidth']=setting.MAX_CAPACITY
			return graph
		except:
	...
	def _save_freebandwidth(self, dpid, port_no, speed):
		...
		if port_state:
			...
			if free_bw==0:
				self.congested=True
			self.current_free_bandwidth[dpid].setdefault(port_no, None)
			self.current_free_bandwidth[dpid][port_no] = free_bw
		else:
