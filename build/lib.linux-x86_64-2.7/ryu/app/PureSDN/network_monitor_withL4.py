...
class NetworkMonitor(app_manager.RyuApp):
	...
	def _flow_stats_reply_handler(self, ev):
		...
		for stat in sorted([flow for flow in body if ((flow.priority not in [0, 65535]) or ((flow.match.get('ipv4_src')) and (flow.match.get('ipv4_dst')) and (flow.match.get('tcp_dst')) and (flow.match.get('tcp_dst'))))],
			...
			key = (stat.priority, stat.match.get('ipv4_src'), stat.match.get('ipv4_dst'),stat.match.get('tcp_src'),stat.match.get('tcp_dst'))
			...
			if(flowDemand>0.1):
				self.flows.append({'ip_src': ip_src, 'ip_dst': ip_dst, 'tcp_src':tcp_src,'tcp_dst':tcp_dst,'demand': flowDemand, 'priority': stat.priority})
			self._save_stats(self.flow_speed[dpid], key, speed, 5)

			# print("%%%%%%%%%%%%%"+str(dpid)+str(self.flows)+"%%%%%%%%%%%%%%%")
			# print(self.flows)
		#print('####'+str(self.flows)+'####')
		if (len(self.statRecord) ==CONF.fanout) and self.flows:
			count=0
			flows = sorted([flow for flow in self.flows], key=lambda flow: (flow['ip_src'], flow['ip_dst'],flow['tcp_src'],flow['tcp_dst'],flow['priority']))
			for flow in flows:
			...
			for link in link_to_port:
				...
				if src_dpid in bw_dict and dst_dpid in bw_dict:
					...
					if graph.has_edge(src_dpid, dst_dpid):
						graph[src_dpid][dst_dpid]['bandwidth'] = int(100/bandwidth)
					else:
					...
					if graph.has_edge(src_dpid, dst_dpid):
						...
						graph.add_edge(src_dpid, dst_dpid)
						graph[src_dpid][dst_dpid]['bandwidth'] = setting.MAX_CAPACITY = 10000
			return graph
		except:
	...
	def _get_free_bw(self, capacity, speed):
		# freebw: Kbit/s
		value=int(max(capacity - speed * 8 / 1000.0, 0)/100)
		if value==0:
			value=1
		return value
	...
	def show_topology(self):
		...
		for node1 in sorted([node for node in _graph.nodes()], key=lambda node: node):
			...
			for node2 in sorted([node for node in _graph.nodes()], key=lambda node: node):
				if (node1, node2) in link_to_port.keys():
					print ('%6s , %s' % (str(link_to_port[(node1, node2)]) , str(_graph[node1][node2]['bandwidth'])))
				else:
					print '%6s' % '/',
			print
		print
