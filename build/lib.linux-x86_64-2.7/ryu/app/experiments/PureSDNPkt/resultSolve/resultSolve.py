...
class SolveData:
    ...
    def plot_line_timeconsume(self):
        xlabel=[12.5,25,37.5,50,62.5,75]
        xlist1=[30.07,48.78,71.47,110.43,160.9,225.8]
        xlist2=[30.08,49.54,75.58,120.53,180.12,260.5]
        xlist3=[28.19,48.54,78.94,135.4,215.5,300.4]
        xlist4=[24.05,44.54,75.58,136.4,224.5,314.6]
        plt.xlabel(u'传输总数据量(GB)', fontproperties=chinese_font, fontsize='large')
        plt.ylabel(u'数据传输完成时间(s)', fontproperties=chinese_font, fontsize='large')
        plt.ylim(0, 320)
        plt.xlim(12.5,75)
        plt.xticks(np.linspace(12.5,75,6))
        plt.yticks(np.linspace(20, 320,21))
        plt.plot(xlabel, xlist1, linewidth=3.0, color='k' ,label='SRMF', ls='--',lineWidth=1.5)
        plt.plot(xlabel, xlist2, linewidth=3.0, color='k',label='NMF',  marker='*',markerSize=8,ls='-',lineWidth=1.5)
        plt.plot(xlabel, xlist3, linewidth=3.0, color='k', label='Hedera', marker='^',markerSize=8,ls='-',lineWidth=1.5)
        plt.plot(xlabel, xlist4, linewidth=3.0, color='k', label='ECMP',ls='-', markerSize=8,lineWidth=1.5)
