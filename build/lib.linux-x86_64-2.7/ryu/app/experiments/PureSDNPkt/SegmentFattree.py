...
import time
import traffic_generation as tx
...
parser.add_argument('--index', dest='idx', type=float, default=0.5, help='index')
...
def createTopo(pod, density, ip="192.168.16.138", port=6653, bw_c2a=100, bw_a2e=100, bw_e2h=100):
    ...
    Controller_Ryu = Popen("ryu-manager --observe-links SRrouting.py --k_paths=%d --weight=hop --fanout=%d --dir=%s --miceIndex=%f --index=%f" % (k_paths, fanout, args.output_dir[2:], args.miceIndex,args.idx), shell=True, preexec_fn=os.setsid)
    ...
    time.sleep(45)
    tx.first_icmp_delay(net,peers1.iperf_peers)
    # tx.traffic_generation(net,iperf_peers,ping_peers)
    CLI(net)
    # peers=[]
    # for p in peers1.iperf_peers:
    #     peer=(p[0],p[1],'700M')
    #     peers.append(peer)
    # print str(peers)
    # random.shuffle(peers)
    # tx.time_comsume(net,peers)
    #
    # time.sleep(10000)


    # # # # Wait until the controller has discovered network topology.
    # closeable(os,Controller_Ryu)
