...
def _GA_Fit(routine):
    ...
    Max_value=setting.MAX_CAPACITY*1024.0
        ...
        for path in paths:
            if path[int((len(path) - 1) / 2)] == core:
                for i in xrange(0, len(path) - 1):
                    graph[path[i]][path[i + 1]]['bandwidth']=graph[path[i]][path[i + 1]].get('bandwidth') - speed*8.0# consider the flow_demand
                    # spare=graph[path[i]][path[i + 1]].get('bandwidth')-(speed*8)/1024
                    if minBand>min(minBand,graph[path[i]][path[i + 1]]['bandwidth']):
                        minBand=min(minBand,graph[path[i]][path[i + 1]]['bandwidth'])
