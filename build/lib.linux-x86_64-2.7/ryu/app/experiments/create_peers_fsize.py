...
parser.add_argument('--fsize', dest='fsize', default='1G', help="every flow size")
		...
		while len(new_peers) < flowsPerHost:
			...
			if probability < edge_prob:
				...
				if (peer != host) and ((host, peer) not in new_peers):
					new_peers.append((host, peer,args.fsize))
	...
	for host in HostList:
		for i in xrange(flowsPerHost):
			...
			while (peer == host or (host, peer) in peers):
				peer = random.choice(HostList)
			peers.append((host, peer,args.fsize))
	return peers
...
def create_peers():
	...
	file_save = open('iperf_peers_fsize.py', 'w')
