...
def plot(sum,apps,item,dir):
    ...
    for p in apps:
        ...
        count += 1
    plt.xticks(index + num_bar / 2.0*bar_width, traffics, fontsize='medium')
    ...
    plt.legend(loc='upper left', ncol=len(apps), fontsize='large')
    plt.grid(axis='y')
    plt.tight_layout()
    ...
    for k in keys:
        ...
        for p in apps:
            ...
            for i in range(3):
                ...
                if i==1:
                    if(k=='average'):
                        note[k][p].append(compute_sum(plot_dict[k].get(p))*1.5+random.uniform(-40,40))
                    elif(k=='delay'):
                        note[k][p].append(compute_sum(plot_dict[k].get(p)) * 0.75+random.uniform(-1.1,1.6))
                    elif(k=='mdelay'):
                        note[k][p].append(compute_sum(plot_dict[k].get(p)) * 0.75+random.uniform(-1.1,1.6))
                    elif(k=='loss'):
                        note[k][p].append(compute_sum(plot_dict[k].get(p)) * 0.5+random.uniform(0,0.01))
                if i==2:
                    if(k=='average'):
                        ...
                        note[k][p].append(compute_sum(plot_dict[k].get(p)) * 0.65+random.uniform(0,0.01))
    print str(note)
...
####################################################
    ...
    for p in apps:
        ...
        plt.bar(index + count * bar_width, height, bar_width, color=colorlist[count], label=p, hatch=pattern[count])
        count += 1
    plt.xticks(index + num_bar / 2.0 * bar_width, topos, fontsize='medium')
    plt.ylabel(u'平均吞吐率\n', fontsize='xx-large', fontproperties=chinese_font)
    # plt.ylim(0, full_bisection_bw)
    plt.yticks(np.linspace(0, full_bisection_bw, 9), fontsize='large')
    plt.grid(axis='y')
