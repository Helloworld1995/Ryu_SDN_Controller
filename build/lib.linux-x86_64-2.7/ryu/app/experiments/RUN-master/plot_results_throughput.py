...
def get_average_bisection_bw(value_dict, traffics, app):
    ...
    for traffic in traffics:
        complete_list.append(value_dict[traffic]['accumulated_throughput'][app][args.duration] / (float(args.duration)*10.0))
...
def plot_results():
    ...
    for app in apps:
        for i in xrange(args.duration + 1):
            for j in xrange(i + 1):
                throughput['SRMF']['accumulated_throughput'][app][i] += throughput['SRMF']['realtime_throughput'][app][j]  # Mbit
                throughput['SRMF']['normalized_total_throughput'][app] =throughput['SRMF']['accumulated_throughput'][app][args.duration] / (full_bisection_bw * args.duration)  # percentage
    ...
    for app in apps:
        temp_list1=get_average_bisection_bw(throughput,traffics,app)
        plot_dict['throughput'][app]=temp_list1
        temp_list2 = get_value_list_2(throughput, traffics, item, app)
