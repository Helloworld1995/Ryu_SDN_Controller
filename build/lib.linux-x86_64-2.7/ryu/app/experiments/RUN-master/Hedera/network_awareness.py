...
class NetworkAwareness(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.access_table = self.create_access_table(
            CONF.fanout)   # {(sw,port):(ip, mac),}
    ...
    def _discover(self):
        i = 0
        if CONF.weight == 'hop':
            period = setting.DISCOVERY_PERIOD
        elif CONF.weight == 'bw':
            period = setting.DISCOVERY_PERIOD2
        while True:
            self.show_topology()
    ...
    def create_access_table(self, fanout):
        ...
        for i in xrange(3001, 3001 + (fanout**2) / 2):
            for j in xrange(fanout / 2 + 1, fanout + 1):
                table[(i, j)] = ('10.%d.0.%d' %
                                 (int(str(i)[-2:]), k), '00:00:00:00:00:%02x' % num)
                num += 1
                k += 1
                if k == fanout / 2 + 1:
                    k = 1
    ...
    def all_k_shortest_paths(self, graph, weight='weight', k=5):
        ...
        for src in _graph.nodes():
            ...
            for dst in _graph.nodes():
                ...
                paths[src][dst] = self.k_shortest_paths(
                    _graph, src, dst, weight=weight, k=k)
        return paths
