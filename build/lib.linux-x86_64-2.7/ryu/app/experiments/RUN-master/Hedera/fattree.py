...
parser.add_argument('--cpu', dest='cpu', type=float,
                    default=1.0, help='Total CPU to allocate to hosts')
    ...
    def createHost(self, NUMBER):
        ...
        for i in xrange(1, NUMBER + 1):
            ...
            self.HostList.append(self.addHost(
                PREFIX + str(i), cpu=args.cpu / float(NUMBER)))
...
def install_proactive(net, topo):
    ...
    for sw in topo.EdgeSwitchList:
        ...
        if topo.pod == 4:
			...
			'group_id=1,type=select,bucket=output:1,bucket=output:2'" % sw  # origin is 1 and 2,i think wrong
        elif topo.pod == 8:
            cmd = "ovs-ofctl add-group %s -O OpenFlow13 \
...
#         server.cmd("iperf -s > %s/%s &" % (args.output_dir, 'server'+filename+'.txt'))
        # Its statistics is useless, just throw away.
    ...
    for src, dest in flows_peers:
        ...
        # Its statistics is useless, just throw away. 1990 just means a great
        # number.

        client.cmd("iperf -c %s -t %d > /dev/null &" %
                   (server.IP(), 1990))
        time.sleep(1)
    ...
    monitor = Process(target=monitor_devs_ng, args=(
        '%s/bwmng.txt' % args.output_dir, 1.0))
    monitor.start()
    for client in (set(topo.HostList) - serversList):   # topo.HostList
        ...
        for server in (set(topo.HostList) - serversList):   # for server in topo.HostList
            server = net.get(server)
            if client != server:
                client.cmd("ping -c 1 -n -q %s >> %s/%s &" %
                           (server.IP(), args.output_dir, 'first_packets.txt'))
    ...
    for client in (set(topo.HostList) - serversList):
        ...
        for server in (set(topo.HostList) - serversList):   # for server in topo.HostList
            server = net.get(server)
            if client != server:
                client.cmd("ping -c %d -i 0.1 -n -q %s >> %s/%s &" % (args.duration *
                                                                      10, server.IP(), args.output_dir, 'successive_packets.txt'))
...
def run_experiment(pod, density, ip="192.168.229.128", port=6653, bw_c2a=10, bw_a2e=10, bw_e2h=10):
    """
...
#     Install proactive flow entries.
    ...
    k_paths = (args.k / 2)**2
    fanout = args.k
    Controller_Ryu = Popen("ryu-manager --observe-links ./Hedera.py --k_paths=%d --weight=hop --fanout=%d" %
                           (k_paths, fanout), shell=True, preexec_fn=os.setsid)

    # Wait until the controller has discovered network topology.
    time.sleep(60)
    print('***********traffic coming!***********')

    # 3. Generate traffics and test the performance of the network.
    traffic_generation(net, topo, iperf_peers.iperf_peers)
#     CLI(net)  # stay in mininet
    # Stop the controller.
    # CLI(net)
    if net.stop():
