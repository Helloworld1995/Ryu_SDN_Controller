...
class NetworkMonitor(app_manager.RyuApp):
    ...
    def _monitor(self):
        ...
        while CONF.weight == 'bw' or CONF.weight == 'hop':
            ...
            for dp in self.datapaths.keys():

                self.port_features.setdefault(dp, {})
                self._request_stats(self.datapaths[dp])
            ...
            if port_no != ofproto_v1_3.OFPP_LOCAL:
                ...
                if len(tmp) > 1:
                    ...
                    period = self._get_period(
                        tmp[-1][3], tmp[-1][4], tmp[-2][3], tmp[-2][4])
                speed = self._get_speed(
                    self.port_stats[key][-1][0], pre, period)
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        for stat in sorted([flow for flow in body if ((flow.priority not in [0, 65535]) and (flow.match.get('ipv4_src')) and (flow.match.get('ipv4_dst')))],
            ...
            key = (stat.priority, stat.match.get(
                'ipv4_src'), stat.match.get('ipv4_dst'))
    ...
    def _get_paths(self, src, dst):
        ...
        temp_paths = shortest_paths.get(src).get(dst)

        for path in temp_paths:
            subpaths_agg.setdefault(path[1], [])
            subpaths_edge.setdefault(path[0], [])
            subpaths_agg[path[1]].append(path)
            subpaths_edge[path[0]].append(path)
        edges = subpaths_edge.keys()
        aggs = subpaths_agg.keys()
        for agg in aggs:
            min_bw_.setdefault(agg, [])
            for path in subpaths_edge[agg]:
                min_bw_[agg].append(
                    self.get_min_bw_of_links(graph, path, min_bw))
            self._group_mod(agg, min_bw)
        ...
        for path in paths:
            ...
            for i in xrange(len(path) - 1):
                ...
                if self.awareness.link_to_port.has_key((path[i], path[i + 1])):
                    src_port = self.awareness.link_to_port[(
                        path[i], path[i + 1])][0]
            ...
            if fitCheck == True:
                GFF_route = path
                self.logger.info("[GFF PATH]%s<-->%s: %s" %
                                 (flow['src'], flow['dst'], path))
                break
        if GFF_route:
            # Install new GFF_path flow entries.
            self.logger.info("[GFF INSTALLING]%s<-->%s: %s" %
                             (flow['src'], flow['dst'], path))
    ...
    def _install_GFF_path(self, GFF_route, match, priority):
        ...
        '''
        flow_info = (match['eth_type'], match['ipv4_src'],
                     match['ipv4_dst'], priority)
        # Install flow entries to datapaths along the path.
        self.install_flow(
            self.datapaths, self.awareness.link_to_port, GFF_route, flow_info)
    ...
    def install_flow(self, datapaths, link_to_port, path, flow_info):
        ...
        for i in xrange(1, len(path) - 2):
            port = self.get_port_pair_from_link(
                link_to_port, path[i - 1], path[i])
            port_next = self.get_port_pair_from_link(
                link_to_port, path[i], path[i + 1])
    ...
    def get_best_path_by_bw(self, graph, paths):
        ...
        # self.capabilities and self.best_paths have no actual utility in this
        # module.
    ...
    def _save_freebandwidth(self, dpid, port_no, speed):
        ...
        if port_state:
            # The true bandwidth of link, instead of 'curr_speed'.
