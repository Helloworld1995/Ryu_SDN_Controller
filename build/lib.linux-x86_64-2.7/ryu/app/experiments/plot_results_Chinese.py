...
def get_throughput(throughput, traffic, app, input_file):
	...
	if app == 'NonBlocking':
		switch = '1001'
	elif app in ['BFlows', 'EFattree', 'ECMP', 'PureSDN', 'Hedera','Normalrouting','SRrouting']:
...
def get_average_bisection_bw(throughput, traffics, app):
	...
	complete_list.append(throughput[traffics]['accumulated_throughput'][app][args.duration] / float(args.duration))
	print complete_list
	for i in xrange(9):
		value_list.append(calculate_average(complete_list[(i * 8): (i * 8 + 8)]))
...
def plot_results():
	...
	"""
	full_bisection_bw = 30.0 * (args.k ** 3 / 4)   # (unit: Mbit/s)
	...
	traffics_brief=['10','100']

	apps = ['ECMP','PureSDN','Hedera']
	throughput = {}

	for traffic in traffics_brief:
		for app in apps:
			# bwmng_file = args.out_dir + '/%s/%s/%s/bwmng.txt' % (args.flows_num_per_host, traffic, app)
			bwmng_file='/home/lee/ryu2/ryu/app/experiments/results3/%s/random/SRrouting/bwmng/bwmng.txt'%traffic
		# 		throughput = get_throughput(throughput, traffic, app, bwmng_file)
			throughput = get_throughput(throughput, 'random', 'Normalrouting', bwmng_file)
	...
	Normalrouting_value_list=get_average_bisection_bw(throughput,'random','Normalrouting')
	print Normalrouting_value_list
	# NonBlocking_value_list = get_average_bisection_bw(throughput, traffics, 'NonBlocking')
	index = np.arange(num_groups) + 0.15
	bar_width = 8
	# plt.bar(0.15, height=ECMP_value_list[0], width=bar_width, color='b', label='ECMP')
	# plt.bar(0.25, PureSDN_value_list[0], bar_width, color='g', label='PureSDN')
	# plt.bar(0.35, Hedera_value_list[0], bar_width, color='b', label='Hedera')
	plt.bar(0.1, Normalrouting_value_list, bar_width, color='b', label='Normalrouting')
