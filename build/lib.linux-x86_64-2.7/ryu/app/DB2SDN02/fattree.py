...
from ryu.app.experiments.iperf_peers import iperf_peers
...
class Fattree(Topo):
	...
	def _set_ovs_protocol_13(self, sw_list):
		for sw in sw_list:
			...
			os.system(cmd)
	def install_queue(self): #添加优先级队列，priority的指越小优先级越高,可参考 https://www.sdnlab.com/19208.html
		for sw in self.EdgeSwitchList:
			for port in ['1','2']:
				cmd = "ovs-vsctl set port %s-eth%s qos=@newqos -- \
								--id=@newqos create qos type=linux-htb \
								other-config:max-rate=1000000000 queues=1=@q1,2=@q2,3=@q3 -- \
...
def run_experiment(pod, density, ip="192.168.16.138", port=6653, bw_c2a=100, bw_a2e=100, bw_e2h=100):
	"""
