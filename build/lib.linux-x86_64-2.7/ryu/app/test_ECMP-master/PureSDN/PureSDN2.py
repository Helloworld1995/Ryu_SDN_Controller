...
class ShortestForwarding(app_manager.RyuApp):
	...
	def get_path2(self, src, dst, weight):
		...
		# Create bandwidth-sensitive datapath graph.
		graph=self.awareness.graph
		graph2 = self.monitor.graph
		
		if weight == self.WEIGHT_MODEL['hop']:
			return nx.shortest_path(graph,src,dst,weight='weight',method='dijkstra')
			...
			path = nx.shortest_path(graph2, src, dst, weight='bandwidth', method='dijkstra')
			return path
		else:
			pass
	...
	def send_flow_mod(self, datapath, flow_info, src_port, dst_port):
		...
		self.add_flow(datapath, 30, match, actions,idle_timeout=3, hard_timeout=0)
		...
		if result:
			...
			if dst_sw:
				...
				# Path has already been calculated, just get it.
					...
					path = self.get_path2(src_sw, dst_sw, weight=self.weight)
					if path==None:
						return
